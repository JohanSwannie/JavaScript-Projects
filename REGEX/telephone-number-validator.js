// Telephone Number Validator - Return true if the passed string looks like a valid US phone
// number. The user may fill out the form field any way they choose as long as it has the
// format of a valid US number. The following are examples of valid formats for US numbers
...
function telephoneCheck(str) {
  const REGEXT = /^(1\s?)?(\d{3}|\(\d{3}\))[\-\s]?\d{3}[\-\s]?(\d{4})$/gm
  let testTelephoneNumber = REGEXT.test(str)
  return testTelephoneNumber;
}
